...
class FixtureController extends \common\controllers\BaseController {
    ...
    public function actionAddhouse() {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            if ($pid) {
                $error = '吐槽成功';
                $this->refresh();
                $notices = array('type' => 2, 'msgtitle' => '操作成功', 'message' => $error, 'backurl' => Url::toRoute('/fixture/house/' . $pid), 'backtitle' => '查看吐槽');
                Yii::$app->getSession()->setFlash('wechat_fail', array($notices));
                $this->redirect(Url::toRoute('/public/notices'));
            } else {
