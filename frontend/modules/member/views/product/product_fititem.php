...
<div class="content">
    <div class="container">
        <div class="tabs">
            <a href="<?= Url::toRoute('/member/product/index') ?>" class="tab-but tab-but-1<?= $status == 0 ? ' tab-active' : ''; ?>">处理中</a>
            <a href="<?= Url::toRoute('/member/product/index') ?>?status=1" class="tab-but tab-but-2<?= $status == 1 ? ' tab-active' : ''; ?>">发货中</a>
            <a href="<?= Url::toRoute('/member/product/index') ?>?status=3" class="tab-but tab-but-3<?= $status == 3 ? ' tab-active' : ''; ?>">已成功</a>
            <a href="<?= Url::toRoute('/member/product/index') ?>?status=2" class="tab-but tab-but-4<?= $status == 2 ? ' tab-active' : ''; ?>">已取消</a>     
        </div>     
    </div>
</div>
